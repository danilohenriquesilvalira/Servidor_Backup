...
import {
  Download, Search, X, RefreshCw, Trash2,
...
export default function Shared() {
  ...
  return (
    <div className="flex flex-col gap-4 lg:h-full lg:overflow-hidden animate-fadeIn pb-10 lg:pb-0 font-sans">
      ...
      <div className={`${CARD_ST} flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 p-6 lg:p-4 flex-shrink-0`}>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              className="input !bg-white border-2 border-gray-100 pl-11 h-12 lg:h-10 text-xs font-black uppercase tracking-widest"
      ...
      <div className={`${CARD_ST} flex-1 flex flex-col lg:overflow-hidden`}>
        ...
        {showNewFolder && (
          <form onSubmit={createSharedFolder} className="p-4 bg-slate-50 border-b border-gray-200 flex gap-3 animate-fadeIn flex-shrink-0">
            <input
              type="text"
              className="input flex-1 !bg-white"
              placeholder="Nome da pasta pública..."
              value={folderName}
              onChange={e => setFolderName(e.target.value)}
              autoFocus
            />
            <button type="submit" className="px-6 rounded-xl bg-[#0B3904] text-white text-[10px] font-black uppercase tracking-widest">
              Criar no Acervo
            </button>
            <button type="button" onClick={() => setNewFolder(false)} className="p-3 rounded-xl hover:bg-white text-gray-400">
              <X size={16} />
            </button>
          </form>
        )}
        ...
        <div className="flex-1 overflow-auto custom-scrollbar">
          {loading ? (
            ...
            <table className="w-full text-sm border-separate border-spacing-0">
              <thead>
                <tr>
                  <th className="th-first">Item Público</th>
                  <th className="th hidden sm:table-cell">Autor</th>
                  <th className="th hidden md:table-cell">Tamanho</th>
                  <th className="th-last text-right px-5">Ações</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {folders.map(folder => (
                  <tr key={folder.id} className="table-row group cursor-pointer" onClick={() => goFolder(folder)}>
                    <td className="td">
                ...
                {files.map(file => {
                  ...
                  return (
                    <tr
                      ...
                      onClick={() => previewable ? setPreview(file) : download(file)}
                    >
                      <td className="td">
                      ...
                      <td className="td">
                        <div className="flex items-center justify-end gap-3" onClick={e => e.stopPropagation()}>
                          <button onClick={() => download(file)} className="p-2.5 rounded-xl bg-white border-2 border-gray-200 text-gray-400 hover:text-black transition-all">
                            <Download size={16} />
                          </button>
                          {canDelete(file) && (
                            <button onClick={() => deleteFile(file.id, file.original_name)} className="p-2.5 rounded-xl bg-white border-2 border-gray-200 text-gray-400 hover:text-red-500 transition-all">
                              <Trash2 size={16} />
                            </button>
                          )}
                        </div>
        ...
        {total > LIMIT && (
          <div className="px-5 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50/60 flex-shrink-0">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <button
                ...
                className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl bg-white border border-gray-200 text-[10px] font-black uppercase tracking-widest disabled:opacity-30 hover:border-gray-400 transition-all"
              >
                Anterior
              </button>
              <button
                ...
                onClick={() => setPage(p => p + 1)}
                className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl bg-[#0B3904] text-white text-[10px] font-black uppercase tracking-widest disabled:opacity-30"
              >
                Próxima
              </button>
            </div>
      ...
      {preview && (
        <PreviewModal
          ...
          onDownload={() => { download(preview); setPreview(null) }}
        />
      )}
    </div>
  )
